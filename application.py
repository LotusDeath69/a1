...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event in log['events']: 
        if event['type'] == 'call':
            src_number = event['src_number']
            dst_number = event['dst_number']
            time = datetime.datetime.strptime(event['time'], "%Y-%m-%d %H:%M:%S")
            if time.month != billing_month: 
                billing_month = time.month 
                new_month(customer_list, time.month, time.year)
            duration = event['duration']
            src_loc = event['src_loc'] # list[0, 1]
            dst_loc = event['dst_loc'] # list[0, 1]
            call = Call(src_number, dst_number, time, duration, src_loc, dst_loc)
            src_cus = find_customer_by_number(src_number, customer_list)
            dst_cus = find_customer_by_number(dst_number, customer_list)
            src_cus.make_call(call)
            dst_cus.receive_call(call)
